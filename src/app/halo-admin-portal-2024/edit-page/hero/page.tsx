...
function SortableSlide({
  ...
  return (
    <div
      ...
      style={style}
      id={`slide-${index}`}
      className="p-4 sm:p-8 rounded-lg border border-dark-200 dark:border-dark-700 hover:border-primary-400 transition-all bg-white dark:bg-dark-800 shadow-sm hover:shadow-md"
    >
      {/* Slide Header */}
      <div className="flex items-center justify-between gap-2 sm:gap-3 mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-dark-200 dark:border-dark-700">
        <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing p-1.5 sm:p-2 hover:bg-dark-100 dark:hover:bg-dark-700 rounded-lg transition-colors flex-shrink-0"
          >
            <GripVertical size={18} className="sm:w-6 sm:h-6 text-dark-400" />
          </div>
          <div className="px-2 py-1 sm:px-4 sm:py-1.5 bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-full text-sm sm:text-base font-semibold flex-shrink-0">
            Slide {index + 1}
          </div>
          <h3 className="text-base sm:text-xl font-semibold text-dark-900 dark:text-white truncate flex-1 min-w-0">
            {slide.title}
          </h3>
        </div>
      </div>
...
export default function HeroEditor() {
  ...
  const fetchHeroContent = async () => {
    try {
      ...
      if (data.heroContent && data.heroContent.slides) {
        // Ensure each slide has a colorScheme, defaulting to 'green' if missing
        const slidesWithColorScheme = data.heroContent.slides.map(
          (slide: HeroSlide) => ({
            ...slide,
            colorScheme: slide.colorScheme || "green",
          })
        );
        setSlides(slidesWithColorScheme);
        console.log("Slides set to:", slidesWithColorScheme);
      }
    } catch (error) {
  ...
  const addSlide = () => {
    ...
    // Scroll to the new slide after a brief delay to allow rendering
    setTimeout(() => {
      const slideElement = document.getElementById(`slide-${newSlideIndex}`);
      if (slideElement) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-dark-50 via-white to-primary-50 dark:from-dark-950 dark:via-dark-900 dark:to-dark-950 pt-24">
      ...
      >
        {/* Header */}
        <div className="mb-6 mt-4 px-4 max-w-7xl mx-auto">
        ...
        <div className="mb-8 overflow-hidden bg-white dark:bg-dark-800 shadow-sm border-y border-dark-200 dark:border-dark-700">
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    >
                      {editMode === `subtitle-${currentSlide}` ? (
                        <input
                          ...
                          autoFocus
                          className="bg-transparent text-primary-300 font-semibold text-sm outline-none text-center min-w-[150px] sm:min-w-[200px]"
                        />
                      ) : (
                    ...
                    >
                      {editMode === `title-${currentSlide}` ? (
                        <input
                          ...
                          onBlur={() => setEditMode(null)}
                          autoFocus
                          className="bg-transparent text-white text-4xl sm:text-5xl lg:text-6xl font-display font-bold outline-none text-center w-full"
                        />
                      ) : (
                        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-display font-bold text-white mb-2 sm:mb-3 leading-tight drop-shadow-2xl">
                          {slide.title}
                        </h1>
                      )}
                    </div>
                    ...
                    <div
                      ...
                      onClick={() => setEditMode(`description-${currentSlide}`)}
                    >
                      {editMode === `description-${currentSlide}` ? (
                        <input
                          type="text"
                          ...
                          onBlur={() => setEditMode(null)}
                          autoFocus
                          className="bg-transparent text-dark-200 text-xl sm:text-2xl outline-none text-center w-full"
                        />
                      ) : (
                        <p className="text-xl sm:text-2xl text-dark-200 mb-3 sm:mb-4 max-w-2xl mx-auto leading-relaxed">
                          {slide.description}
                        </p>
                      )}
                    </div>
                    ...
                    {/* CTA Button */}
                    <div className="flex flex-col gap-4 sm:gap-5 justify-center items-center mt-2">
                      <div
        ...
        {/* Manage Slides Section - Contained */}
